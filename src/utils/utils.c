...
void swapRows(size_t cols, uint32_t* matrix, size_t swap_row_1, size_t swap_row_2) {
...
// TODO: remove
void printMatrix(uint32_t rows, uint32_t cols, uint32_t* matrix) {
  for (uint32_t i = 0; i < rows; ++i) {
    for (uint32_t j = 0; j < cols; ++j) {
      printf("%3x ", matrix[(i * cols) + j]);
    }
    puts("");
  }
}
...
void gaussEliminationModulo(uint32_t rows, uint32_t cols, uint32_t* matrix) {
  uint32_t row_pivot = 0;
  uint32_t col_pivot = 0;
  // Casting uint32_t* to pointer of uint32_t[cols] which allows use of double
  // subscript element access.
  uint32_t (*m)[cols] = (uint32_t (*)[cols])matrix;

  while (row_pivot < rows && col_pivot < cols) {
    // Find the k-th pivot.
    uint32_t i_max = row_pivot;
    uint32_t max = m[i_max][col_pivot];
    for (uint32_t i = i_max + 1; i < rows; ++i) {
      if (m[i][col_pivot] > max) {
        max = m[i][col_pivot];
        i_max = i;
      }
    }
    ...
    if (m[i_max][col_pivot] == 0) ++col_pivot;
      ...
      if (row_pivot != i_max) swapRows(cols, matrix, row_pivot, i_max);
      // Do for all rows below pivot.
      for (uint32_t i = row_pivot + 1; i < rows; ++i) {
        uint32_t factor = (m[i][col_pivot] * inverseMod257[m[row_pivot][col_pivot]]) % MOD;
        // Fill with zeros the lower part of pivot column.
        m[i][col_pivot] = 0;
        // Do for all remaining elements in current row.
        for (uint32_t j = col_pivot + 1; j < cols; ++j) {
          int64_t val = ((int64_t)m[i][j] - (int64_t)m[row_pivot][j] * factor) % MOD;
          if (val < 0) val += MOD;
          m[i][j] = val;
        }
      }
...
void solveSystem(uint32_t rows, uint32_t cols, uint32_t* matrix, uint8_t* coeficients) {
  if (cols != rows + 1) {
  ...
  for (int64_t i = rows - 1; i >= 0; --i) {
    int64_t coef = m[i][cols - 1];
    for (int64_t j = cols - 2; j >= i + 1; --j) {
      coef = (coef - (int64_t)m[i][j] * coeficients[j]) % MOD;
    }
